...
class Actor:
    def __init__(self, rank, game):
        ...
        self.model_target = NatureCNN(self.state_shape[0], self.action_dim).cuda()
    ...
    def sample(self, epsilon, state_dict, target_state_dict):
        ...
        for step in range(steps):
            ...
            st_next = torch.from_numpy(np.array(obs_next)).float().cuda() / 255.0
            at = torch.from_numpy(np.array(action)).long().cuda()
            tt = torch.from_numpy(np.array(done)).float().cuda()
            rt = torch.from_numpy(np.array(reward)).float().cuda()

            with torch.no_grad():
                q_next = self.model_target(st_next)
                q_next_online = self.model(st_next)
                q_next = q_next.gather(1, q_next_online.argmax(dim=-1).unsqueeze(-1)).squeeze(-1)
                q_target = rt + discount * (1.0 - tt) * q_next

            q = qs.gather(1, at.unsqueeze(-1)).squeeze(-1)
...
def train(game):
    ...
    agent = Agent(game)
    sample_ops = [a.sample.remote(1.0, agent.model.state_dict(), agent.model_target.state_dict()) for a in actors]
    ...
    while True:
        ...
        if rank < num_actors:
            ...
            sample_ops.append(actors[rank].sample.remote(epsilon, agent.model.state_dict(), agent.model_target.state_dict()))
            RRs += Rs
            QQs += Qs
