...
AgentZero es un framework ligero de aprendizaje por refuerzo distribuido basado en PyTorch y DeepMind Launchpad.
...
Aquí, FPS (frames por segundo) se refiere a los frames recopilados y guardados en el buffer de replay por segundo. Con 4 saltos de frame (frame skip), su FPS es de 5000x4= **20000++** según DeepMind. Otras implementaciones suelen quedarse por debajo de 100 FPS tras la fase de exploración. La verdadera limitación (cuello de botella) radica en la transferencia de datos desde la CPU a la GPU.
...
Ejecución multihilo con Launchpad:
```bash
...
```bash
# Nuestra implementación actual incluye: c51, qr, mdqn, iqr, fqf
uv run python -m agent0.deepq.launch learner.algo=c51
```

<!-- 
Ejecutar como en rainbow:
```bash
# exp_name especificará el directorio de checkpoints bajo $HOME/ray_results
python -m agent0.deepq.run --double_q --dueling --noisy --priortize --n_step 3 --game atari47 --algo c51 --exp_name atari_rainbow
``` -->

## Resultado de ejecución de ejemplo @10M frames
| exp_name                 | commit | algo | juego          | media     | std      | máx   | mín  | tamaño | frames   |
|--------------------------|--------|------|----------------|-----------|----------|-------|------|--------|----------|
