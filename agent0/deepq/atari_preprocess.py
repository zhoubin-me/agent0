...
class AtariPreprocessing(gym.Wrapper, gym.utils.RecordConstructorArgs):
    ...
    def __init__(
        ...
        if self.env.unwrapped.get_action_meanings()[1] == 'FIRE' and \
            len(self.env.unwrapped.get_action_meanings()) >= 3:
    ...
    def step(self, action):
        ...
        for t in range(self.frame_skip):
            ...
            if self.terminal_on_life_loss:
                terminated = terminated or life_loss
    ...
    def reset(self, **kwargs):
        ...
        if self.terminal_on_life_loss and not self.game_over:
            for i in range(3):
                if i == 0 or (i > 0 and self.fire_reset):
                    _, _, terminated, truncated, reset_info = self.env.step(i)
                if terminated or truncated:
                    terminal_info = self.env.reset(**kwargs)
                    reset_info.update(terminal_info)
                    break
        ...
        for i in range(noops):
            if i < noops - 2 or not self.fire_reset:
                _, _, terminated, truncated, step_info = self.env.step(0)
            else:
                act = i - noops + 3
                _, _, terminated, truncated, step_info = self.env.step(act)
